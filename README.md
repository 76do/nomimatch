...
- 未登録ユーザー
	...
	- 飲み会依頼を送信できる。（チャットは不可）
- 登録ユーザー
    - ログイン ログアウト機能(電話番号,LINE,Facebook)
	- 自分宛の飲み会依頼フォーム入力画面へのリンクを発行できる。(リンクをクリックすると登録ユーザー宛の飲み会依頼フォーム入力画面へ遷移する。未登録ユーザーはリンクからチャットリンクなしの飲み会依頼を送信できる。)
	- 登録ユーザーは上記リンクからチャットリンクありの飲み会依頼を送信できる。
    - フォームで飲み会の詳細（希望解散時間・希望人数・飲み会の雰囲気・店のURL・フリー記述）を記載することができる。フォームの入力内容から飲み会依頼メッセージを作成する。
    - 飲み会依頼メッセージはログイン媒体に応じた方法で送信される。（電話番号ログインの場合はSMS、LINEログインの場合はLINE。Facebookログインの場合はFacebookメッセンジャー)
